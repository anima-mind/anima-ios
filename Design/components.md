...
## Settings
Grouped lists in bordered radius-8 containers (row h48, hairline separators). Sections: Account (name/email or "No account", Sign in with Apple when signed out, Sign out, Delete account with typed "eliminar" confirmation — deletes only the identity, the mind stays local), Model (API key masked · Keychain, monthly budget + 3px progress), Cost by turn type (label · bar · $), Permissions (Allowed / Off), Mind (cycles · p, Simulate one night, Replay onboarding).
